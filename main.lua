...
function WordReference:onDispatcherRegisterActions()
  Dispatcher:registerAction("wordreference_action", {category="none", event="show_language_pair_dialog", title=_("Word Reference"), general=true,})
end
...
function WordReference:addToMainMenu(menu_items)
  menu_items.wordreference = {
    ...
    callback = function()
      self:show_language_pair_dialog()
    end,
  }
end
...
function WordReference:show_language_pair_dialog()
  local menu
...
function WordReference:lookup_and_show(phrase)
  ...
  local snippet = WebRequest.parse_wr_html_for_snippet(res.body)
  if not snippet then
    UIManager:close(progressMessage);
  ...
  local result_dialog = TextViewer:new {
    title = phrase,
    text = result_text,
    text_type = "lookup",
    text_font_size = 14,
  ...
  UIManager:nextTick(function() UIManager:show(result_dialog) end)
end
